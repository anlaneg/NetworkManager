           ...
           GError **error)
{
	...
	GBytes *hwaddr;
	const uint8_t *hwaddr_arr = NULL;
	gsize hwaddr_len;
	guint16 arptype;
	sd_dhcp_lease *lease = NULL;
	GBytes *client_id;
	gs_unref_bytes GBytes *client_id_new = NULL;
	const uint8_t *client_id_arr = NULL;
	...
	ifindex = nm_dhcp_client_get_ifindex (client);
	...
	r = sd_dhcp_client_set_ifindex (priv->client4, ifindex);
	if (r < 0) {
           ...
           GError **error)
{
	...
	r = sd_dhcp6_client_set_ifindex (priv->client6, ifindex);
	if (r < 0) {
		nm_utils_error_set_errno (error, r, "failed to set ifindex: %s");
		goto errout;
	}
